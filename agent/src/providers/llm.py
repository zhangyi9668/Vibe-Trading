...
def _sync_provider_env(provider: str | None = None) -> None:
    ...
    provider_name = (provider or config.langchain_provider).lower()

    if provider_name in {"openai-codex", "openai_codex"}:
        codex_url = config.openai_codex_base_url
    ...
    key_env, base_env = provider_env_names(provider_name, config.langchain_model_name)
...
def build_llm(
    *,
    ...
    """
    if not (provider is not None and provider.lower() in {"openai-codex", "openai_codex"}):
        _sync_provider_env(provider)
    config = get_env_config().llm
    provider_name = (provider or config.langchain_provider).lower()
    name = model_name or config.langchain_model_name.strip()
    if not name:
